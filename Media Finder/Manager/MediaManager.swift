...
class MediaManager {
    ...
    let mediaTable = Table("media2") // Create Table with name
    ...
    func setupConnection(){
        do{
            ...
            let fileUrl = documentDirectory.appendingPathComponent("media2").appendingPathExtension("sqlite3") // database name and extension
    ...
    func insertMedia(mediaArr: [Media]){
        let Email = UserDefaultManager.shared().userEmail!
    for media in mediaArr{
            let insertmedia = self.mediaTable.insert(self.artworkUrl100 <- media.artworkUrl100,self.artistName <- media.artistName ?? "",self.trackName <- media.trackName ?? "",self.previewUrl <- media.previewUrl,self.kind <- media.kind ?? "",self.longDescription <- media.longDescription ?? "", self.email <- Email)
            do{
    ...
    func deleteMedia(){
        let Email = UserDefaultManager.shared().userEmail!
        let media = self.mediaTable.filter(self.email == Email)
        let deleteMedia = media.delete() // what to be deleted
        do{
            try self.dataBase.run(deleteMedia) // try to delete data
            print("Deleted Media")
        }catch{
            print(error)
            return
    ...
    func getMediaDatabase(completion: (_ mediaArr: [Media]?)-> Void){
        ...
        let Email = UserDefaultManager.shared().userEmail!
        do{
            let media = try self.dataBase.prepare(self.mediaTable.filter(self.email == Email))
            for mediaIndex in media{
                let mediaData = Media(artworkUrl100: "\(mediaIndex[self.artworkUrl100])",
                    artistName: "\(mediaIndex[self.artistName])",
                    trackName: "\(mediaIndex[self.trackName])",
                    previewUrl: "\(mediaIndex[self.previewUrl])",
                    kind: "\(mediaIndex[self.kind])",
                    longDescription: "\(mediaIndex[self.longDescription])",
                    email: "\(mediaIndex[self.email])")
